...
def phase1_update(source_prob, wiki_list, keywords, n_iter, verbosity, lambd = 0, log = None, mode_of_operation = 4):
        '''
        ...
        for kw in keywords:
                ...
                if not log == None:
                    if mode_of_operation == 4:
                        source_prob = utilities.textrank(source_prob, wiki_content, lambd = lambd, log = log+'_'+str(j)+'_'+kw)
                    elif mode_of_operation == 5:
                        source_prob = utilities.rake(source_prob, wiki_content, lambd = lambd, log = log+'_'+str(j)+'_'+kw)     
                    else:
                        source_prob = utilities.tfidf(source_prob, wiki_content, lambd = lambd, log = log+'_'+str(j)+'_'+kw, mode_of_operation=mode_of_operation, return_term=0)
                
                else:
                    if mode_of_operation == 4:
                        source_prob = utilities.textrank(source_prob, wiki_content, lambd = lambd, log = None)
                    elif mode_of_operation == 5:
                        source_prob = utilities.rake(source_prob, wiki_content, lambd = lambd, log = None)     
                    else:
                        source_prob = utilities.tfidf(source_prob, wiki_content, lambd = lambd, log = None, mode_of_operation=mode_of_operation, return_term=0)
                
                if verbosity : print "\n\n---------\n"

                for i in range(len(source_prob)):
                        if verbosity : print wiki_list[i][0], source_prob[i]/source_prob[0]
                        if not log == None: logFile.write(wiki_list[i][0]+" : "+str(source_prob[i]/source_prob[0])+'\n')
...
def test_run(keywords, verbosity, lambd = 0, log = None):
        '''
        ...
        if not log == None:
                ...
                source_prob = phase1_update(source_prob, wiki_list, keywords, 1, verbosity, lambd = lambd, log = log+'/'+log, mode_of_operation = mode_of_operation)
        else:
                source_prob = phase1_update(source_prob, wiki_list, keywords, 1, verbosity, lambd = lambd, log = None, mode_of_operation = mode_of_operation)
...
if __name__ == "__main__":
        log = "test" #raw_input("Enter a name for the run : ")
        keywords = ["Delhi", "dog", "mars", "atom"]
        #keywords = ["delhi"]
        verbosity = True
        test_run(keywords, verbosity, log = log)
